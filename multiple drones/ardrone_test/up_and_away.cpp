...
Modified code for Multiple Ardrone.  
...
int main(int argc, char** argv)
{
	...
	pub_empty1 = node.advertise<std_msgs::Empty>("/ardrone_1/takeoff", 1); /* Message queue length is just 1 */
	...
	pub_empty3 = node.advertise<std_msgs::Empty>("/ardrone_3/takeoff", 1);
 	while (ros::ok()) 
				...
				while ((double)ros::Time::now().toSec()< time_start+5.0) /* Send command for five seconds*/
					{ 
					pub_empty1.publish(emp_msg); /* launches the drone 1*/
